...
export interface SecurityLog {
  ...
  event: "FAILED_LOGIN" | "UNAUTHORIZED_ACCESS" | "CREDENTIALS_REVOKED" | "SESSION_TIMEOUT" | "DATABASE_RESET";
  username: string;
  details: string;
  ipAddress: string;
}
...
export function loadSecurityLogs(): SecurityLog[] {
  const stored = localStorage.getItem(STORAGE_KEY_SECURITY_LOGS);
  if (!stored) return [];
  try {
...
export function addSecurityLog(
  ...
  localStorage.setItem(STORAGE_KEY_SECURITY_LOGS, JSON.stringify(updatedLogs));
  return updatedLogs;
}
...
export function clearSecurityLogs(): void {
  localStorage.removeItem(STORAGE_KEY_SECURITY_LOGS);
}
