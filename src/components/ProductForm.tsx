...
export const ProductForm: React.FC<ProductFormProps> = ({
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const loadPromises = fileList.map((file: File) => {
      return new Promise<string | null>((resolve) => {
        if (file.size > 4 * 1024 * 1024) {
          showToast(`"${file.name}" must be smaller than 4MB`, "error");
          resolve(null);
          return;
        }
    ...
    Promise.all(loadPromises).then((results) => {
      ...
      if (validImages.length > 0) {
        ...
        showToast(`${validImages.length} image(s) loaded successfully!`, "success");
      }
    });
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    showToast(`"${sanitizedName}" successfully added to catalog!`, "success");
              ...
              if (fileList.length > 0) {
                const loadPromises = fileList.map((file: File) => {
                  return new Promise<string | null>((resolve) => {
                    ...
                    const reader = new FileReader();
                    reader.onload = (ev) => resolve(ev.target?.result ? (ev.target.result as string) : null);
                    reader.onerror = () => resolve(null);
                    reader.readAsDataURL(file);
                  });
                });
                Promise.all(loadPromises).then((results) => {
                  ...
                  if (validImages.length > 0) {
                    setImageUrls((prev) => [...prev, ...validImages]);
                    showToast(`Dropped ${validImages.length} image(s) successfully!`, "success");
                  }
                });
              } else {
