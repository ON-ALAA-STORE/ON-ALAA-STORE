...
export const ProductModal: React.FC<ProductModalProps> = ({ product, onClose, onAdd, lang }) => {
  ...
  const [reviews, setReviews] = useState<any[]>(() => {
    const stored = localStorage.getItem(`alaa_store_reviews_${product.id}`);
    if (stored) {
      try {
  ...
  const handleSubmitReview = (e: React.FormEvent) => {
    ...
    localStorage.setItem(`alaa_store_reviews_${product.id}`, JSON.stringify(updatedReviews));
      ...
      >
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-slate-500 leading-relaxed font-bold">
            {getProductField(lang, product, "desc") || (lang === "ar" ? "لا يوجد وصف متوفر لهذا المنتج." : "No custom product description available.")}
          </p>
        </div>
